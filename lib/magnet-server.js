...
function MagnetServer (client, opts = {}) {
  ...
  function onRequest (req, res) {
    ...
    function handleRequest () {
      ...
      var magnetLink = ""
      if (path.includes(".")) {
        magnetLink = Buffer.from(path.split('.')[0], 'base64').toString();
      } else {
        magnetLink = Buffer.from(path, 'base64').toString();
      }

      // TODO: there is probably a race condition between when the link is added and when the metadata has been fetched.
      if (client.get(magnetLink) === null) {
        client.add(magnetLink, function(torrent) {
            console.log("Got files for magnet: " + magnetLink);
            var file = torrent.files.find(function (file) {
                return file.name.endsWith('.mp4') || file.name.endsWith('.mkv');
            });
            console.log("Going to serve: " + file);
            serveFile(file);
        });
      } else {
